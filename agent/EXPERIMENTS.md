...
## Stage25 result and stage26

Stage25 commit `175ae2a`, run `5ccc0d71-af80-4f98-b393-1b67c704548c`, failed hidden `incorrect_output`. All public cases passed with essentially unchanged throughput, consistent with fallback there; this does not establish which hidden path failed. Full-output warmup comparison was not sufficient. The entire experiment is reverted from engine/. Its source and protocol tests remain under agent/candidates/stage25; four-token followup drafts are not submitted.

Stage26 starts from verified stage21 and only fuses prefill per-head Q/K RMSNorm into rotary/cache writes. Separate native projection GEMMs remain unchanged. The kernel keeps the worked RMSNorm FP32 reduction and BF16 cast boundaries, with its default fusion setting rather than changing variance rounding. Every layer compares Q, K and V bit-for-bit with separate native operations in warmup; on disagreement that layer retains the established separate-normalization path. No speculative backend is included.
