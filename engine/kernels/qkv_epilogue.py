"""BF16 Q/K norm, rotary, and token-major KV write for prefill and decode."""
...
def _qkv_epilogue(QKV, QW, KW, COS, SIN, POS, Q, K, V,
                  CAPACITY: tl.constexpr, TOKENS: tl.constexpr, QEPS: tl.constexpr, KEPS: tl.constexpr):
    row = tl.program_id(0)
    batch = row // TOKENS
    token = row % TOKENS
...
def qkv_epilogue(qkv, attn, position_embeddings, position, key, value, capacity):
    """Contiguous [B,T,6144] BF16 input; T>1 requires an empty cache."""
    batch, tokens = qkv.shape[:2]
    query = torch.empty((batch, tokens, 32, 128), dtype=qkv.dtype, device=qkv.device)
    cosine, sine = position_embeddings
    _qkv_epilogue[(batch * tokens, 40)](
        qkv, attn.q_norm.weight, attn.k_norm.weight, cosine, sine,
        position, query, key, value, capacity, tokens,
